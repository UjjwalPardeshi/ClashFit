...
> **What shipped, 4 Sep 2026: both.** The boss is a rigged, animated glTF model rendered by
> Filament — 42 nodes, 16 meshes, five clips, 54 KB — generated by `tools/make-boss.py` rather
> than licensed, so the joint hierarchy matches the combat model exactly and there is no
> licence to defend. The Compose-drawn boss described here is still in the build as the
> automatic fallback: if Filament will not start on a device, the fight still happens with a
> complete boss rather than an apology.
